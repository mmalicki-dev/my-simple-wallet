...
describe("Transaction endpoints", () => {
  ...
  describe("POST /api/transaction/", () => {
    it("creates a transaction and returns 201", async () => {
      const res = await request(app)
        ...
        .set("Authorization", `Bearer ${token}`)
        .send({ amount: 100, type: "income", category: categoryId });
    ...
    it("updates user balance on create", async () => {
      await request(app)
        .post("/api/transaction/")
        .set("Authorization", `Bearer ${token}`)
        .send({ amount: 50, type: "income", category: categoryId });
  ...
  describe("PUT /api/transaction/:id", () => {
    it("updates a transaction", async () => {
      ...
      const res = await request(app)
        ...
        .set("Authorization", `Bearer ${token}`)
        .send({ amount: 200, type: "income", category: categoryId });
  ...
  describe("DELETE /api/transaction/:id", () => {
    ...
    it("reverses user balance on delete", async () => {
      const createRes = await request(app)
        .post("/api/transaction/")
        .set("Authorization", `Bearer ${token}`)
        .send({ amount: 100, type: "income", category: categoryId });
